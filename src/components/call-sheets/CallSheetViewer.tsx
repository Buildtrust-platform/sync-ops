...
export default function CallSheetViewer({ callSheetId, onEdit }: CallSheetViewerProps) {
  ...
  const loadCallSheet = async () => {
    try {
      ...
      // Load call sheet
      const { data: callSheetData } = await client.models.CallSheet.get({ id: callSheetId });
      if (callSheetData) {
        setCallSheet(callSheetData);
      }

      // Load scenes
      const { data: scenesData } = await client.models.CallSheetScene.list({
        filter: { callSheetId: { eq: callSheetId } },
      });
      if (scenesData) {
        setScenes(scenesData.sort((a, b) => (a.sortOrder || 0) - (b.sortOrder || 0)));
      }

      // Load cast
      const { data: castData } = await client.models.CallSheetCast.list({
        filter: { callSheetId: { eq: callSheetId } },
      });
      if (castData) {
        setCast(castData.sort((a, b) => (a.sortOrder || 0) - (b.sortOrder || 0)));
      }

      // Load crew
      const { data: crewData } = await client.models.CallSheetCrew.list({
        filter: { callSheetId: { eq: callSheetId } },
      });
      if (crewData) {
        setCrew(crewData.sort((a, b) => (a.sortOrder || 0) - (b.sortOrder || 0)));
      }
    } catch (error) {
    ...
    <div className="bg-white shadow-lg rounded-lg overflow-hidden max-w-5xl mx-auto">
      ...
      <div className="bg-indigo-600 text-white px-8 py-6">
        ...
        {onEdit && (
          <button
            onClick={onEdit}
            className="mt-4 inline-flex items-center px-4 py-2 border border-white text-sm font-medium rounded-md text-white hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-white"
          >
            Edit Call Sheet
          </button>
        )}
