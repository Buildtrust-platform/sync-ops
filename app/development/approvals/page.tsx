...
// Map project data to approval items
function mapProjectToApprovals(project: Schema['Project']['type']): ApprovalItem[] {
  const items: ApprovalItem[] = [];

  // Map brief approvals if brief exists
  if (project.brief) {
    const brief = project.brief as any; // Type assertion needed for nested object

    // Budget approval
    if (!brief.approvedByFinance) {
      items.push({
        ...
        type: 'BUDGET',
        status: 'PENDING',
        requestedBy: project.projectOwnerEmail || 'Unknown',
        requestedAt: new Date(project.createdAt || Date.now()).toLocaleDateString(),
        assignedTo: 'Finance Team',
        priority: 'HIGH',
        description: 'Approval needed for project budget',
      });
    }

    // Legal approval
    if (!brief.approvedByLegal) {
      items.push({
        id: `${project.id}-legal`,
        ...
        priority: 'MEDIUM',
        description: 'Legal review and approval required',
      });
    }

    // Creative approval
    if (!brief.approvedByProducer) {
      items.push({
        id: `${project.id}-creative`,
        ...
        assignedTo: 'Producer',
        priority: 'HIGH',
        description: 'Creative direction approval needed',
      });
    }
  }
...
export default function ApprovalsPage() {
  ...
  const fetchApprovals = useCallback(async () => {
    ...
    try {
      ...
      // Fetch projects with briefs for this organization
      ...
      // Map all projects to approval items
      const allApprovals = projectsData.flatMap(project => mapProjectToApprovals(project));
